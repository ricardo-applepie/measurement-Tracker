...
test("testing measurements state ", () => {
  ...
  const value = 2 + 2;
  expect(store.getState().measurements.measurements.length).toBeGreaterThan(1);
  expect(store.getState().measurements.measurements[0]).toEqual({
    id: 1,
    weight: "50kg",
    date: "Mon Aug 18 2014 21:11:54 GMT+0200 (Central European Summer Time)",
  });

  expect(store.getState().measurements.measurements[0].id).toBe(1);
  expect(store.getState().measurements.measurements[1].id).toBe(2);
  expect(store.getState().measurements.measurements[0].weight).toBe("50kg");
  expect(store.getState().measurements.measurements[1].weight).toBe("80kg");
});
